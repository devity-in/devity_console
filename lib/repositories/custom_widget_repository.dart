...
class CustomWidgetRepository {
  /// Creates a new [CustomWidgetRepository]
  ...
  static const String _widgetsKey = 'custom_widgets';
  ...
  Future<List<CustomWidget>> loadCustomWidgets(String projectId) async {
    try {
      ...
      final cachedData =
          await cacheService.getCachedData('$_widgetsKey:$projectId');
      if (cachedData != null) {
  ...
  Future<void> saveCustomWidget(String projectId, CustomWidget widget) async {
    ...
    await cacheService.cacheData(
      '$_widgetsKey:$projectId',
      [...widgets, widget].map((w) => w.toJson()).toList(),
    );
  }
  ...
  Future<void> deleteCustomWidget(String projectId, String widgetId) async {
    ...
    await cacheService.cacheData(
      '$_widgetsKey:$projectId',
      updatedWidgets.map((w) => w.toJson()).toList(),
    );
  }
